...
import { z } from 'zod';
import { atualizarDepartamento, criarDepartamento } from '../../lib/api/departamentos';
...
export function DepartamentoFormDialog({ open, departamento, onClose }: DepartamentoFormDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setErroGeral(null);
      reset(departamento ? { descricao: departamento.descricao, ativo: departamento.ativo } : DEFAULT_VALUES);
    }
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData) => {
      if (modoEdicao) {
        return atualizarDepartamento(departamento!.id, data);
      }
      return criarDepartamento({ descricao: data.descricao });
    },
    onSuccess: () => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>{modoEdicao ? 'Editar departamento' : 'Novo departamento'}</DialogTitle>
      <Box
        component="form"
        onSubmit={(e) =>
